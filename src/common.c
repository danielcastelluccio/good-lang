...
Value get_type(Context *context, Node *node) {
	Node_Types *node_types = hmget(context->node_types, context->static_value_id);
	if (node_types == NULL) {
		node_types = malloc(sizeof(Node_Types *));
		*node_types = NULL;
		hmput(context->node_types, context->static_value_id, node_types);
	}

	Value result = hmget(*node_types, node);
	if (result.value == NULL && context->static_value_id != 0) {
		size_t saved_static_argument_id = context->static_value_id;
		context->static_value_id = 0;
...
Node_Data *get_data(Context *context, Node *node) {
	Node_Datas *node_datas = hmget(context->node_datas, context->static_value_id);
	if (node_datas == NULL) {
		node_datas = malloc(sizeof(Node_Datas *));
		*node_datas = NULL;
		hmput(context->node_datas, context->static_value_id, node_datas);
	}

	Node_Data *result = hmget(*node_datas, node);
	if (result == NULL && context->static_value_id != 0) {
		size_t saved_static_argument_id = context->static_value_id;
		context->static_value_id = 0;
